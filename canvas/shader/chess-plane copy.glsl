...
float intersect(in Plane p, in Ray ray) {
    ...
    if (t < 0.0)          // <-- descarta o que está atrás da câmera
        return INFINITY;
...
/// ****************************************************************************
/// Chess Pattern function
/// ****************************************************************************
vec4 chess(vec2 uv, float quadrados )
{
    float cx = step(0.5, fract( 0.5 * uv.x * quadrados));
    float cy = step(0.5, fract( 0.5 * uv.y * quadrados)); 
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    Plane plane;
    plane.normal 	  = vec3(cos(2.0 * iTime * 3.1415), 10.0, abs(sin(iTime * 3.1415) - 1.0));
    plane.d			  = 5.0;
	...
	if (tPlano == INFINITY) 
    	...
    	fragColor = chess(at(ray,tPlano).xz,1.0);
    }
}